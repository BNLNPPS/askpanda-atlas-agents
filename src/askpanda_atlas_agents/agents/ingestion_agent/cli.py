...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog='askpanda-ingestion-agent',
        description='Periodic BigPanda / PanDA data ingestion agent.',
    )
    p.add_argument(
        '--config', '-c',
        default='src/askpanda_atlas_agents/resources/config/ingestion-agent.yaml',
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        _version = importlib.metadata.version("askpanda-atlas-agents")
    except importlib.metadata.PackageNotFoundError:
        _version = "unknown"
    logger.info(
        "askpanda-ingestion-agent  version=%s  python=%s.%s.%s",
        _version,
